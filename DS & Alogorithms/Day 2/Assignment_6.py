...
class Performance:
    ...
    def change_position(self, child):
        first = self.__children_list.get_head()
        second = self.__children_list.get_head()
        
        while(first.get_next()):
            second = second.get_next()
            if first.get_next().get_next():
                first = first.get_next().get_next()
        
        self.__children_list.delete(child)
        
        self.__children_list.insert(child, second.get_data())
